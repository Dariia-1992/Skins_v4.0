...
public class ModDetailsFragment extends Fragment {
    ...
    private static String GooglePlayStoreAppLink = "https://play.google.com/store/apps/details?id=" + BuildConfig.APPLICATION_ID;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        circleIndicator.setVisibility(skin.getBigImages().length > 1 ? View.VISIBLE : View.GONE);
        ...
        shared.setOnClickListener(v -> {
            new Intent(Intent.ACTION_VIEW, Uri.parse(GooglePlayStoreAppLink));
        });
    ...
    private void updateFavoriteState() {
        ...
        if (skin.isFavorite() == true)
            image.setImageResource(R.drawable.is_favourite_button);
        else
            image.setImageResource(R.drawable.favourite_button_false);
...
//                : R.drawable.favourite_button_false);
    ...
    private void showDialogEmptySuccessfully() {
        //MinecraftDontInstallDialog dialog = new EmptySuccessfullyDialog();
        //dialog.show(getChildFragmentManager(), EmptySuccessfullyDialog.class.getSimpleName());
    }
    ...
    private final View.OnClickListener onFavoriteClicked = v -> {
        if (getContext() == null || skin == null)
            return;
        if (skin.isFavorite() == false) {
            skin.setFavorite(true);
            updateFavoriteState();
        }
        else {
            skin.setFavorite(false);
            updateFavoriteState();
        }
//
//        FavoritesManager manager = FavoritesManager.getInstance();
//        boolean isFavorite = manager.isFavorite(getContext(), skin.getId());
//        if (isFavorite)
//            manager.removeFavorite(getContext(), skin.getId());
//        else
//            manager.addToFavorite(getContext(), skin.getId());
